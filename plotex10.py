...
for s_idx, s in enumerate(s_vals):
    ...
    for n_idx, n in enumerate(tau_vals):

        for N_idx, N in enumerate(N_vals):

            file = (
                RESULTS_DIR
                / f"out_simu_results_ex10_B{B}_s{s}_tau{n}_N{N}.xlsx"
            )
...
for s_idx, s in enumerate(s_vals):
    ...
    for N_idx, N in enumerate(N_vals):
        ...
        for n_idx in range(len(tau_vals)):
