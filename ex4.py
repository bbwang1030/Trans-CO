...
def generate_data_with_outliers(K=5, s=40, n=150, N=800, o=50):
    ...
    w = np.random.uniform(low=-2.0, high=2.0, size=K) # w = (3/2, 3/4, 0, 0, -5/4)
    ...
    # 生成delta
    S = np.random.choice(np.arange(1, p), size=s_delta, replace=False)
    delta = np.zeros(p)
    ...
    for k in range(K):
        ...
        sigma_k = 1
        eps_k = np.random.normal(0, sigma_k, N)  # σk²=1
        gamma = np.zeros(N)
        # 随机选择o个不同的索引位置
    ...
    X_target = np.random.multivariate_normal(mu_k, sigma,n)  # 协方差矩阵Σ=I
...
for num_s,s in enumerate(s_):
    ...
    for num_N,N in enumerate(N_):
        ...
        for num_n,n in enumerate(n_):
            ...
            # 保存到 Excel
            with pd.ExcelWriter(RESULTS_DIR / f"out_simu_results_ex4_B{B}_s{s}_n{n}_N{N}.xlsx") as writer:
