...
def generate_data_with_outliers(K=5, s=40, n=150, N=800, o=50, K_irrelevant = 1):
    # 固定参数
    p = 100
    h = 6
    K_relevant = K-K_irrelevant
    print("K_relevant", K_relevant)
    w = np.random.uniform(low=-2.0, high=2.0, size=K_relevant)
...
for num_s,s in enumerate(s_):
    ...
    for num_N,N in enumerate(N_):
        print("N", N)
        for num_K_irrelevant,K_irrelevant in enumerate(K_irrelevants):
            print("K_irrelevant", K_irrelevant)
            mse_results = []
            mse_results_ipod = []
            mse_results_Trans_CO = []
            results_list = []
            for b in range(B):
                setting_id = (num_s * len(N_) + num_N) * len(K_irrelevants) + num_K_irrelevant
                rep_seed = BASE_SEED + setting_id * B + b
                set_random_seed(rep_seed)
                print(b, "seed =", rep_seed)
                # 生成模拟数据（假设generate_data返回X, y, beta）
                sparse_data = generate_data_with_outliers(K=K, s=s, n=n, N=N, o=o,K_irrelevant = K_irrelevant)
            ...
            # 保存到 Excel
            with pd.ExcelWriter(RESULTS_DIR / f"out_simu_results_ex9_B{B}_s{s}_n{n}_N{N}_{K_irrelevant}.xlsx") as writer:
