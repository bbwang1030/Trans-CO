...
for s_idx, s in enumerate(s_vals):
    ...
    for n_idx, n in enumerate(support_vals):

        for N_idx, N in enumerate(N_vals):

            file = (
                RESULTS_DIR
                / f"out_simu_results_ex8_B{B}_s{s}_n200_N{N}_support{n}.xlsx"
            )
...
for s_idx, s in enumerate(s_vals):
    ...
    for N_idx, N in enumerate(N_vals):
        ...
        for n_idx in range(len(support_vals)):
