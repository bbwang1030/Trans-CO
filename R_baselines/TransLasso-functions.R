...
library(glmnet)
agg.fun <- function(B, X.test, y.test, total.step = 20, selection = FALSE) {
  B <- as.matrix(B)
  ...
  pred_mat <- X.test %*% B
  loss_vec <- colSums((as.vector(y.test) - pred_mat)^2)

  if (selection) {
    khat <- which.min(loss_vec)
  ...
  stable_weight <- function(z) {
    ...
    z[!is.finite(z)] <- min(z[is.finite(z)])
    z <- z - max(z)
  ...
  theta.hat <- stable_weight(-loss_vec / 2)
  theta.old <- theta.hat

  beta <- as.numeric(B %*% theta.hat)
  beta.ew <- beta

  for (ss in seq_len(total.step)) {
    pred_beta <- as.vector(X.test %*% beta)

    z <- -colSums((as.vector(y.test) - pred_mat)^2) / 2 +
      colSums((pred_beta - pred_mat)^2) / 8

    theta.hat <- stable_weight(z)

    beta <- as.numeric(B %*% theta.hat * 1 / 4 + 3 / 4 * beta)

    diff_theta <- sum(abs(theta.hat - theta.old))

    if (is.finite(diff_theta) && diff_theta < 10^(-3)) {
      break
    }
...
las.kA<-function(X, y, A0, n.vec, lam.const=NULL, l1=T){
  ...
  if(size.A0 > 0){
    ...
    if(is.null(lam.const)){
      cv.init<-cv.glmnet(X[ind.kA,], y.A, nfolds=5, lambda=seq(1,0.001,length.out=20)*sqrt(2*log(p)/length(ind.kA)))
    ...
    # cv.delta<-cv.glmnet(x=X[ind.1,],y=y[ind.1]-X[ind.1,]%*%w.kA, lambda=seq(1,0.001,length.out=20)*sqrt(2*log(p)/length(ind.1)))
    ...
    cv.init<-cv.glmnet(X[1:n.vec[1],], y[1:n.vec[1]], nfolds=5, lambda=seq(1,0.001,length.out=20)*sqrt(2*log(p)/n.vec[1]))
